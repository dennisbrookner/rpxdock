...
def test_layer_hier_3comp(hscore, bodyC6, bodyC3, bodyC2):
    ...
    sampler = rp.sampling.hier_multi_axis_sampler(spec, [[0, 40], [0, 40], [0, 40]], flip_components=True)

    result = rp.search.make_multicomp(bodies, spec, hscore, rp.hier_search, sampler, **kw)

    result.dump_pdbs_top_score(hscore=hscore,
                               **kw.sub(nout_top=5,
                                        output_prefix='test_layer_hier_3comp_test',
    ...
    rp.search.result.result_to_tarball(ref, 'test_layer_hier_2comp.result.txz', overwrite=True)
    rp.search.assert_results_close(result, ref)
...
def test_cage_termini_dirs(hscore, bodyC3, bodyC2, pose_C3_1na0, pose_C2_REFS10):
    ...
    for i, dir_pair in enumerate(dirs):
        ...
        sampler = rp.sampling.hier_multi_axis_sampler(spec, [100, 130],
                                                      flip_components=kw.flip_components,
                                                      force_flip=kw.force_flip)

        result[i] = rp.search.make_multicomp([bodyC3, bodyC2], spec, hscore, rp.hier_search, sampler, **kw)
    ...
    # rp.search.result.result_to_tarball(result, f'test_cage_termini_dirs.result.txz', overwrite=True)
...
def test_cage_term_access(hscore, term_mod_C3_and_C2):
    ...
    for i, bodies in enumerate(body_list):
        ...
        result[i] = rp.search.make_multicomp([bodies[0], bodies[1]], spec, hscore, rp.hier_search, sampler,
                                             **kw)
    for j in range(len(result)):
        for k in range(j + 1, len(result)):
            assert not (result[j].__eq__(result[k]))
